...
public class ColorPickerWidget implements Drawable, Element, Selectable {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(context.getMatrices().peek().getPositionMatrix(), "Color Picker", x + 10, y + 10, 0xFFFFFF);
        ...
        for (int i = 0; i < COLORS.length; i++) {
            ...
            if (COLORS[i] == selectedColor) context.drawBorder(colorX - 2, colorY - 2, COLOR_SIZE + 4, COLOR_SIZE + 4, 0xFFFFFFFF);
            if (isMouseOver(colorX, colorY, COLOR_SIZE, COLOR_SIZE, mouseX, mouseY)) context.drawBorder(colorX - 1, colorY - 1, COLOR_SIZE + 2, COLOR_SIZE + 2, 0xFF888888);
        }

        int currentColorY = startY + ((COLORS.length / COLORS_PER_ROW) + 1) * (COLOR_SIZE + 5);
        context.drawTextWithShadow(context.getMatrices().peek().getPositionMatrix(), "Current:", x + 10, currentColorY, 0xFFFFFF);
    ...
    public void appendNarrations(NarrationMessageBuilder builder) {
        builder.put(Text.literal("Color picker"));
    }
